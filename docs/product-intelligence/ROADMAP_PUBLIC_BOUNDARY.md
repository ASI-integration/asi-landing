...
**This document is governed only by Maturity (and, downstream of it, Public Claim Status), never by Strategic Intent or Roadmap Horizon.** `CAPABILITY_INVENTORY.md` tracks four independent fields per capability: what's built (Maturity), what may be said (Public Claim Status, registered in `CLAIMS_REGISTER.md`), what ASI wants and by what mechanism (Strategic Intent — `CORE_NOW` / `NEXT_BUILD` / `ROADMAP_CONFIRMED` / `RESEARCH` / `PARTNER_OR_INTEGRATE` / `HOLD` / `NOT_STRATEGIC`), and when it's expected to move (Roadmap Horizon — `NOW` / `NEXT` / `LATER` / `RESEARCH` / `UNDECIDED`). A capability marked `NEXT_BUILD` with horizon `NEXT` — even one explicitly confirmed by the owner — does not move up a single tier in this document until its Maturity actually changes. `ROADMAP_CONFIRMED` is an even weaker signal for public-copy purposes than `NEXT_BUILD`: it means only "the owner agrees this belongs in the target system eventually," carrying no sequencing information at all. Wanting to build something, having decided it belongs on the roadmap, or even having scheduled it next, is not evidence that it exists. The Website Editor must never write copy that leaks strategic intent or roadmap timing as if it were a current or near-term capability (e.g. "ASI next replaces your lock app" is a `NEXT_BUILD`/`LATER` intent-and-horizon statement, not a truthful present-tense or even beta claim, and belongs — if anywhere — only in the "planned/future" tier below, worded as a direction, not a promise or a date).
