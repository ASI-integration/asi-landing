...
Map of public **Russian** (`asi-global.ru`) surfaces prepared for YooKassa merchant review.
Live payments and production deploy are **not** enabled by this work.

> Replaces the mixed scope of PR #286: this document and the accompanying code are **RU-host only**.
> Shared/non-RU `SiteFooter` must not link to RU payment/legal routes or show Julia merchant identity.
...
## Legal / contacts routes (RU host only)
...
RU footer/nav: `RuComplianceFooter`, `RuBottomQuickLinks`, `RuPublicNavHeader`.
Non-RU `components/site/SiteFooter`: **no** RU legal/payment links.

## VERIFIED seller data (owner-confirmed, publishable on RU surfaces only)
...
- Target host: `www.asi-global.ru` / `asi-global.ru`
- Visual brand reference: `guestautopilot.com` (do not publish RU merchant identity there)
- Focused tests: `src/config/__tests__/ru-merchant-host-isolation.test.ts`

## Explicitly out of scope here

- Enabling `YOOKASSA_ENABLED=true`
- Live charges, production deploy
- Inventing postal index, apartment number, OGRN/OGRNIP
- Publishing seller details on western/non-RU marketing pages
- RU visual redesign to guestautopilot system (later PR)
