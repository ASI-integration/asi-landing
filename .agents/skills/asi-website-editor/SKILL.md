...
6. **Verify** — run the deterministic site auditor (`npm run site:audit -- --base-url <host><route>`) in its safest read-only mode against the edited route, run only the focused tests for that route within the `AGENTS.md` testing budget, run ESLint on touched files only, run typecheck, and run `git diff --check`. A deterministic audit pass is a verification dependency of this Skill, not proof of editorial quality on its own — this Skill does not duplicate the auditor's rules.
