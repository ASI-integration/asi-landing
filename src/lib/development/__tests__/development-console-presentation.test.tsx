...
describe('development console traffic-light presentation', () => {
  ...
  it('shows merged terminal success state in green with disabled action and no blocked headline', () => {
    ...
    expect(presentation.headline).toBe(MERGE_GATE_HEADLINE.merged);
    expect(presentation.tone).toBe('merged');
    expect(presentation.headline).not.toBe(MERGE_GATE_HEADLINE.blocked);
    ...
    expect(html).toContain(MERGE_GATE_HEADLINE.merged);
    expect(html).not.toContain(MERGE_GATE_HEADLINE.blocked);
    expect(html).toContain('data-merge-gate-hero="merged"');
