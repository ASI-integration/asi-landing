...
function mockListTasksClient(rows: Record<string, unknown>[]) {
  ...
  createClient.mockReturnValue({ rpc: vi.fn(), from });
  return { from, select, eqClient, eqConversation, order, limit };
}
