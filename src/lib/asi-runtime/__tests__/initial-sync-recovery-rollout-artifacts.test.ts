...
const migrationSha256 = '6fbc176b21006d258d4f0253d538c46b66862f0e7752137dda62bc8a88d811ba';
...
describe('Initial Sync Recovery v1 production rollout artifacts', () => {
  it('pins the only production migration workflow to exact repository bytes and target', () => {
    ...
    const migration = fs.readFileSync(path.join(repoRoot, migrationPath));
    ...
    expect(createHash('sha256').update(migration).digest('hex')).toBe(migrationSha256);
