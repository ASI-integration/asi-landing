...
function rowToState(row: AccountLifecycleRow): LifecycleState {
  return {
...
export async function getAccountLifecycle(accountId: string): Promise<LifecycleState> {
  return rowToState(await fetchAccountRow(accountId));
}

/**
 * Card attach step. Called from the SetupIntent-succeeded webhook only —
 * never from a client-submitted request — since server-side state must be
...
export async function markCardVerified(
  ...
  if (verified.changed || started.changed || row.stripe_payment_method_id !== stripeRefs.stripePaymentMethodId) {
 ...
 * always explicit (never auto-inferred from "some data arrived") — the
 * caller (an authenticated internal endpoint) supplies acceptanceEvidence.
 * Fails closed if the requirement was never defined — acceptance always
 * requires an existing requirement row, it never fabricates one on the fly.
 *
 * If this is the last required requirement to reach `accepted`, this same
 * call fires the IntegrationAccepted domain event: integration_ready_at is
 * set and the 14-day trial begins in the same operation, per the approved
 * lifecycle. Repeating this call for an already-accepted requirement is a
 * no-op — it does not reset the trial window (tests I).
 */
...
/**
 * paid_active transition. Re-validates every condition server-side —
