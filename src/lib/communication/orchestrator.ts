...
import { manageInquiryFlow, bridgeInquiryToReservation } from './inquiry-flow';
...
export async function processMessage(envelope: InboundMessageEnvelope): Promise<ProcessResult> {
  ...
  try {
    ...
    // For unmatched contacts: track inquiry state and create booking handoffs.
    // For newly matched contacts: bridge any open inquiry to the stay-flow.
    // Both calls are fire-and-forget — they never block the guest reply path.
    if (commContext.reservation.status === 'unmatched') {
      manageInquiryFlow({
        chatId,
    ...
    } else if (commContext.reservation.status === 'matched' && commContext.reservation.reservationId) {
      bridgeInquiryToReservation(chatId, commContext.reservation.reservationId, identity.guestId).catch(() => {});
    }
