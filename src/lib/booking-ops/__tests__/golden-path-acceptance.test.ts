...
const state = { metadata: { acceptance_safe: true } as Record<string, unknown>, status: 'inquiry', propertyId: 'property-1' as string | null, stage: null as string | null, events: new Set<string>(), failType: '', realSends: 0 };
...
function query(table: string) {
  ...
  const api: Record<string, unknown> = {
    ...
    then: (resolve: (value: unknown) => unknown) => resolve(table === 'booking_ops_worker_tasks' ? { data: ['cleaner','linen_worker','consumables','inspector'].map((assigned_role) => ({ assigned_role, status: 'completed' })), error: null } : { data: [], count: table === 'booking_ops_communication_deliveries' ? state.realSends : 0, error: null }),
  };
  return api;
}
...
describe('OPS v17.2 golden path acceptance runner', () => {
  beforeEach(() => { state.metadata = { acceptance_safe: true }; state.status = 'inquiry'; state.propertyId = 'property-1'; state.stage = null; state.events.clear(); state.failType = ''; state.realSends = 0; vi.clearAllMocks(); });
