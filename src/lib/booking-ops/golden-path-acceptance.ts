...
function plan(row: ReservationRow, stage: string | null, missing: string[]): GoldenPathReport {
  return {
    ...
    finalLifecycleStage: stage, domainEvents: 0, workerTasks: {}, readinessResult: null, blockers: missing,
...
export async function runGoldenPathAcceptance(input: { identifier: string; accountId: string; actorId: string; dryRun?: boolean; confirm?: boolean; featureEnabled?: boolean }): Promise<GoldenPathReport> {
  ...
  const completed = after.stage === 'closed' && after.processingErrors.length === 0 && communications.realSends === 0 && steps.every((step) => step.status === 'PASS');
  if (completed) {
  ...
  const tasks = await taskCounts(row.id);
  return {
    ...plan(row, before.stage, missing), dryRun: false, finalReservationStatus: completed ? 'checked_out' : row.normalized_status,
