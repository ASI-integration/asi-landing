...
export function sanitizeRuPublicFactor(line: string): string | null {
  ...
  const replacements: Array<[RegExp, string]> = [
    [
      ...
      'Рядом есть отдельные деловые точки, но нет крупной точки спроса уровня БЦ, вокзала или делового кластера.',
    ],
    [
      /Рядом только локальные офисные сигналы[^.]*?деловой профиль не подтверждён\.?/u,
      'Рядом есть отдельные офисы, но крупной точки спроса (БЦ, вокзал, деловой кластер) поблизости нет.',
    ],
    [
    ...
    [
      ...
      'В радиусе 1 км нет крупных объектов, которые сами по себе тянули бы спрос (БЦ, вокзал, деловой кластер).',
    ],
    [
      /«Сильный» диапазон требует не менее двух независимых магнитов — один сигнал недостаточен\.?/u,
      'Поблизости только одна сильная точка спроса.',
    ],
    [
      /Деловой профиль не подтверждён сильными магнитами \(вторичный кластер\);\s*оценка ограничена для публичного вывода\.?/u,
      'Деловые объекты есть, но без крупной точки спроса уровня БЦ или вокзала.',
    ],
    [
