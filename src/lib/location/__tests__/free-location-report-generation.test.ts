...
describe('generateFreeLocationReport', () => {
  it('creates, saves, loads, and returns a public-safe free report', async () => {
    ...
    expect(result.report).toMatchObject({
      ...
      keyDemandDrivers: [
        'Метро в пешей доступности: объект проще продвигать для гостей без автомобиля. Ориентир по карте: около 420 м.',
        'Повседневная инфраструктура рядом: гостям проще закрывать бытовые задачи без долгих поездок. Ориентир по карте: около 300 м.',
      ],
      mainRisks: [
        'Экономика и ставка аренды не рассчитаны в бесплатной версии.',
        'Конкуренция рядом не разобрана подробно.',
        'Пешеходный и коммерческий трафик оценён только предварительно.',
      ],
      nearbyStrongObjects: [
        {
          summaryRu: 'Метро в пешей доступности: объект проще продвигать для гостей без автомобиля. Ориентир по карте: около 420 м.',
        },
        {
          summaryRu: 'Повседневная инфраструктура рядом: гостям проще закрывать бытовые задачи без долгих поездок. Ориентир по карте: около 300 м.',
        },
      ],
    });
