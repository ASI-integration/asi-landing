...
const NO_PMS_CHECKLIST = [
  ...
  'Уточнить каналы',
  'Выбрать менеджер каналов или временный ручной режим',
...
const CLARIFY_CHECKLIST = [
  ...
  'Уточнить наличие менеджера каналов',
];
...
const NEXT_STEP = {
  ...
  hasPmsGeneric:
    'Уточнить менеджер каналов, запросить доступ или приглашение и выбрать один тестовый объект для безопасного подключения.',
  noPmsManual:
    'Помочь выбрать менеджер каналов или начать с базовой структуры объекта и каналов.',
  choosingPms:
    'Собрать текущие каналы и требования, затем предложить подходящий сценарий подключения менеджера каналов.',
  support:
    'Ответить на вопрос поддержки и при необходимости перевести в заявку.',
  clarify:
    'Запросить недостающие данные по объектам, каналам и менеджеру каналов, чтобы продолжить квалификацию.',
} as const;
