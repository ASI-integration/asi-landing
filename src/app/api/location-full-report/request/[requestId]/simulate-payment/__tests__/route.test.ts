...
describe('POST /api/location-full-report/request/[requestId]/simulate-payment', () => {
  it('marks a paid-required report request as unlocked without creating a YooKassa payment', async () => {
    ...
    mockProcessPaidReportRequest.mockResolvedValue({
      ...
      status: 'pdf_ready',
      preliminary_report_url: '/ru/location-report/sample?requestId=request-1&view=preliminary',
      final_report_url: '/ru/location-report/sample?requestId=request-1',
      pdf_url: '/ru/location-report/sample/pdf?requestId=request-1',
