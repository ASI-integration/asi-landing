...
async function load(token: string) {
  ...
  const task = await supabase.from('booking_ops_worker_tasks').select('id,booking_id,object_id,assigned_role,status,deadline,checklist,notes,photo_attachments,issue_report,started_at,completed_at').eq('id', link.data.task_id).single();
