...
export async function POST(req: Request) {
  ...
  if (!input) {
    return NextResponse.json({ ok: false, error: 'property_guest_and_dates_required' }, { status: 400 });
  }

  try {
    const result = await createChannelReservation(auth.ctx, input);
    const ok = result.available || result.idempotent || result.status === 'pending';
    const status = ok ? 201 : 409;
