...
describe('RU-DESIGN-03 early-access visual migration', () => {
  ...
  it('uses free-pilot commercial chronology and application CTA, not pre-pilot checkout', () => {
    ...
    expect(page).toContain('Заявка. Бесплатная настройка.');
    expect(page).toContain('Затем 14 дней на реальном объекте.');
    expect(page).toContain("label: 'Сейчас в пилоте'");
    expect(page).toContain("label: 'Позже'");
    expect(page).toContain('Что входит в пилот');
    ...
    expect(page).toContain('От заявки до решения о продолжении');
    expect(page).toContain('Автоматического перехода на оплату нет');
