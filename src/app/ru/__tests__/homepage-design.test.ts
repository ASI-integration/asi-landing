...
describe('RU homepage one-page client flow', () => {
  ...
  it('delivers the complete one-page client journey on /ru', () => {
    ...
    expect(home).toContain('ruComplianceRoutes.offer');
    expect(home).toContain('ruComplianceRoutes.privacy');
    expect(home).toContain('ruComplianceRoutes.contacts');
    expect(home).toContain('Политика конфиденциальности');
    expect(home).toContain('ASI Global © 2026');
    expect(home).toContain('@ASI_Support_Bot');
    expect(home).toContain('support@asi-global.ru');
  ...
  it('keeps homepage header on same-page anchors and reuses the production form', () => {
    const home = readSrc('src/app/ru/page.tsx');
