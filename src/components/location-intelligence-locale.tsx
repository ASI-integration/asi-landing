...
> = {
  en: {
    ...
    loadingSteps: [
      ...
      'Finding transport and demand magnets…',
      'Analyzing demand and risks…',
  ...
  ru: {
    ...
    loadingSteps: [
      'Проверяем адрес…',
      'Ищем транспорт и магниты…',
      'Анализируем спрос и риски…',
    ...
    marketTooltips: {
      ...
      revpar: 'Ориентир RevPAR: оценка ADR × оценка загрузки.',
    },
    incomeTitle: 'Оценка дохода в месяц',
    incomeSuffix: '/ мес',
    incomeDisclaimer1: 'До расходов и комиссий управления',
    incomeDisclaimer2: 'Оценка по открытым данным — не гарантированная «рыночная правда»',
    ...
    envConfidence: (c) =>
      c === 'high' ? 'Высокая уверенность по открытым данным'
      : c === 'medium' ? 'Средняя уверенность по открытым данным'
      : 'Низкая уверенность по открытым данным',
  },
};
...
const MAGNET_WHY_RU: Record<string, string> = {
  ...
  attraction:      'популярное место — постоянный гостевой интерес',
