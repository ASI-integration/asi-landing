...
export { normalizeFreeReportFactors };
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-12">
        <section className="rounded-3xl border border-slate-800/70 bg-slate-900/25 p-7 sm:p-10">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-400">{content.summaryReasonRu}</p>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          <section className="rounded-2xl border border-slate-800/70 bg-slate-900/20 p-6 lg:col-span-2">
            <h2 className="text-2xl font-bold text-white">Сигналы спроса</h2>
          ...
          <section className="rounded-2xl border border-slate-800/70 bg-slate-900/20 p-6">
            ...
            <p className="mt-3 text-sm leading-relaxed text-slate-300">{content.recommendationRu}</p>
            <div className="mt-5">
              <p className="text-sm font-semibold text-white">В подробном отчёте</p>
              <ul className="mt-3 space-y-2 text-sm text-slate-300">
                {content.paidPreviewItemsRu.map(item => (
        ...
        <section className="mt-6 rounded-2xl border border-slate-800/70 bg-slate-900/20 p-6">
          ...
          <ul className="mt-4 space-y-3">
            {content.risksAndLimitationsRu.map(item => (
        ...
        <section className="mt-6 rounded-2xl border border-slate-800/70 bg-slate-900/20 p-6">
          <h2 className="text-2xl font-bold text-white">Дополнительный потенциал</h2>
          <p className="mt-3 text-sm font-semibold text-slate-300">{content.commercialPreview.leadRu}</p>
          <ul className="mt-4 space-y-3">
            {content.commercialPreview.itemsRu.map(item => (
              <li key={item} className="flex gap-3 text-slate-300">
                <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-indigo-400" />
                <span className="leading-relaxed">{item}</span>
              </li>
