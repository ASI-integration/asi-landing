...
export function MediaCard({
  ...
  const inner = (
    <div className="group flex flex-col justify-between h-full p-6 sm:p-7 bg-asi-paper border border-asi-border rounded-sm transition-colors hover:border-asi-gold">
      <div>
        <span className="text-[11px] font-sans font-semibold uppercase tracking-[0.18em] text-asi-gold">{label}</span>
      ...
      {action === 'view' ? (
        ...
        <span className="mt-6 text-sm text-asi-navy/40 font-sans">Coming soon</span>
      )}
    </div>
  );
