...
export function DevelopmentOwnerGuard({ children }: { children: React.ReactNode }) {
  const router = useRouter();
  const { session, loading } = useSession();

  useEffect(() => {
    if (loading) return;
    if (session?.user && session.isDevelopmentOwner !== true) {
  ...
  if (session.isDevelopmentOwner !== true) {
    return (
      <div className="mx-auto max-w-lg rounded-xl border border-slate-200 bg-white p-6 text-center shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600 leading-relaxed">
          Консоль разработки ASI доступна только владельцу из отдельного allowlist.
        </p>
        <Link
          href="/dashboard"
