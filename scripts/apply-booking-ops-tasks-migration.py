#!/usr/bin/env python3
"""Apply Booking Ops Tasks v1 migration via SUPABASE_DB_URL or DATABASE_URL."""
...
SQL = (
    ...
    / "supabase/migrations/20260628000003_booking_ops_tasks_v1.sql"
).read_text(encoding="utf-8")

PROBE_COLUMNS = ["id", "task_type", "status", "booking_ops_record_id"]
...
def probe(url: str, key: str) -> tuple[bool, str]:
    ...
    select = ",".join(PROBE_COLUMNS)
    r = httpx.get(
        f"{base}/rest/v1/booking_ops_tasks?select={select}&limit=1",
        headers=headers,
        timeout=20,
        trust_env=False,
    )
