...
const REQUIRED_CONFIRM = 'RUN_COMM_OPS_ACCEPTANCE';
const RUNNER_TS = resolve(__dirname, 'communication-ops-live-acceptance.runner.ts');
...
function ensureSupabaseEnv(dotenvState) {
  ...
  console.error('[communication-ops-acceptance] FAIL: missing required configuration.');
  if (dotenvState.missingFile) {
    console.error(`[communication-ops-acceptance] missing file: .env.local (${ENV_FILE})`);
  }
  console.error(`[communication-ops-acceptance] missing env: ${missing.join(', ')}`);
  console.error('[communication-ops-acceptance] need at least one of SUPABASE_URL or NEXT_PUBLIC_SUPABASE_URL.');
  process.exit(1);
}
...
function reportRunnerResult(result) {
  ...
  if (code === 0) {
    console.log('[communication-ops-acceptance] PASS');
    process.exit(0);
  }

  console.error(`[communication-ops-acceptance] FAIL: runner exited with code ${code}`);
  if (result.signal) {
    console.error(`[communication-ops-acceptance] signal: ${result.signal}`);
  }
  process.exit(code);
}
...
function main() {
  try {
    const confirm = process.env.COMM_OPS_ACCEPTANCE_CONFIRM?.trim();
    if (confirm !== REQUIRED_CONFIRM) {
      console.log(
        `[communication-ops-acceptance] SKIPPED: no changes. Set COMM_OPS_ACCEPTANCE_CONFIRM=${REQUIRED_CONFIRM} to run live acceptance.`,
      );
      process.exit(0);
    }
