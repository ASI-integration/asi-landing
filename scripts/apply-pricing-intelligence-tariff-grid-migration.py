#!/usr/bin/env python3
"""Apply Pricing Intelligence & Tariff Grid v1 migration via SUPABASE_DB_URL or DATABASE_URL."""
...
SQL = (
    ...
    / "supabase/migrations/20260701180000_pricing_intelligence_tariff_grid_v1.sql"
).read_text(encoding="utf-8")

TABLES = [
    "booking_pricing_profiles",
    "booking_property_audience_profiles",
    "booking_pricing_market_signals",
    "booking_tariff_grid_days",
    "booking_pricing_recommendation_runs",
]
...
def probe(url: str, key: str) -> tuple[bool, str]:
    ...
    r = httpx.get(
        f"{base}/rest/v1/booking_pricing_profiles?select=id&limit=1",
        headers=headers,
        timeout=20,
        trust_env=False,
    )
...
def main() -> int:
    ...
    if ok:
        ...
        return 0
    print("MIGRATION_STATUS=failed_probe", file=sys.stderr)
    return 3
