-- Guest Lifecycle synthetic acceptance cleanup schema fix.
--
-- Additive replacement only. Communication deliveries and auto-send attempts
-- are owned through communication_intent_id -> booking_ops_communication_intents.
-- Do not address those indirect descendants through an optional denormalized
-- booking_id column. All manifest, ownership, confirmation, dry-run, and
-- zero-residue protections remain unchanged.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'deliveries', (SELECT count(*)
      FROM public.booking_ops_communication_deliveries delivery
      JOIN public.booking_ops_communication_intents intent
        ON intent.id = delivery.communication_intent_id
     WHERE intent.booking_ops_record_id = p_booking_ops_record_id),
    'attempts', (SELECT count(*)
      FROM public.booking_ops_communication_auto_send_attempts attempt
      JOIN public.booking_ops_communication_intents intent
        ON intent.id = attempt.communication_intent_id
     WHERE intent.booking_ops_record_id = p_booking_ops_record_id),
