-- Staging-only. Do not apply to production Supabase. Not part of supabase/migrations.
...
-- Preserves columns, indexes, constraints, RPCs, RLS, and service_role grants from 20260724120000_asi_chat_runtime_bridge_v1.sql.
-- Does not CREATE/ALTER/DROP objects in public; primary staging application tables stay untouched.
-- Owner apply (asi-staging SQL editor only):
--   1. Run this file against the existing staging Supabase project.
--   2. Dashboard → Settings → API → Exposed schemas: add runtime_bridge (keep public and graphql_public).
-- Staging app env: ASI_RUNTIME_BRIDGE_SUPABASE_SCHEMA=runtime_bridge
-- Staging ASI_RUNTIME_BRIDGE_SUPABASE_URL / SERVICE_ROLE_KEY may point at the same asi-staging project.
-- Production still requires an isolated Bridge project and schema public (default).

CREATE SCHEMA IF NOT EXISTS runtime_bridge;
COMMENT ON SCHEMA runtime_bridge IS 'Free-tier staging exception: Bridge shares the staging Supabase project but uses dedicated runtime_bridge schema. Production requires isolated Bridge storage.';
REVOKE ALL ON SCHEMA runtime_bridge FROM PUBLIC;
REVOKE ALL ON SCHEMA runtime_bridge FROM anon, authenticated;
GRANT USAGE ON SCHEMA runtime_bridge TO authenticator;
GRANT USAGE ON SCHEMA runtime_bridge TO service_role;
...
CREATE TABLE runtime_bridge.asi_runtime_bridge_owner_gates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  task_id UUID NOT NULL REFERENCES runtime_bridge.asi_runtime_bridge_tasks(id) ON DELETE CASCADE,
  client_id TEXT NOT NULL CHECK (length(client_id) BETWEEN 3 AND 100),
...
CREATE INDEX idx_asi_runtime_bridge_queue
  ON runtime_bridge.asi_runtime_bridge_tasks(status, available_at, created_at, id);
CREATE INDEX idx_asi_runtime_bridge_chat_identity
  ON runtime_bridge.asi_runtime_bridge_tasks(client_id, chatgpt_task_id, conversation_id);
CREATE INDEX idx_asi_runtime_bridge_pending_gates
  ON runtime_bridge.asi_runtime_bridge_owner_gates(client_id, status, created_at);
CREATE UNIQUE INDEX idx_asi_runtime_bridge_owner_decision_once
  ON runtime_bridge.asi_runtime_bridge_owner_gates(client_id, decision_id)
  WHERE decision_id IS NOT NULL;
CREATE UNIQUE INDEX idx_asi_runtime_bridge_single_running
  ON runtime_bridge.asi_runtime_bridge_tasks ((true)) WHERE status = 'running';

CREATE FUNCTION runtime_bridge.submit_asi_runtime_bridge_task(
  p_client_id TEXT,
...
CREATE FUNCTION runtime_bridge.claim_asi_runtime_bridge_task(
  p_client_id TEXT,
  p_runner_id TEXT,
...
BEGIN
  ...
  WITH expired_gates AS (
    UPDATE runtime_bridge.asi_runtime_bridge_owner_gates
    SET status = 'expired'
    WHERE client_id = p_client_id AND status = 'pending'
      AND (request->>'expiresAt')::timestamptz <= now()
    RETURNING task_id
  )
  UPDATE runtime_bridge.asi_runtime_bridge_tasks
  SET status = 'failed',
...
CREATE FUNCTION runtime_bridge.heartbeat_asi_runtime_bridge_task(
  p_client_id TEXT,
  p_runner_id TEXT,
  p_task_id UUID,
  p_lease_token UUID,
...
CREATE FUNCTION runtime_bridge.complete_asi_runtime_bridge_task(
  p_client_id TEXT,
  p_runner_id TEXT,
  p_task_id UUID,
  p_lease_token UUID,
...
CREATE FUNCTION runtime_bridge.gate_asi_runtime_bridge_task(
  p_client_id TEXT,
  p_runner_id TEXT,
  p_task_id UUID,
  p_lease_token UUID,
...
AS $$
DECLARE
  v_gate runtime_bridge.asi_runtime_bridge_owner_gates%ROWTYPE;
BEGIN
  ...
  INSERT INTO runtime_bridge.asi_runtime_bridge_owner_gates(task_id, client_id, task_cycle, request)
...
CREATE FUNCTION runtime_bridge.expire_asi_runtime_bridge_owner_gates(p_client_id TEXT)
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog
...
CREATE FUNCTION runtime_bridge.fail_asi_runtime_bridge_task(
  p_client_id TEXT,
  p_runner_id TEXT,
  p_task_id UUID,
  p_lease_token UUID,
...
CREATE FUNCTION runtime_bridge.decide_asi_runtime_bridge_owner_gate(
  p_client_id TEXT,
  p_task_id UUID,
  p_gate_id UUID,
  p_decision_id TEXT,
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM runtime_bridge.asi_runtime_bridge_owner_gates
    WHERE client_id = p_client_id AND decision_id = p_decision_id AND id <> p_gate_id
  ) THEN
    RAISE EXCEPTION 'decision_conflict';
  END IF;
  SELECT * INTO v_gate FROM runtime_bridge.asi_runtime_bridge_owner_gates
  ...
  IF v_deduplicated THEN
    SELECT * INTO v_task FROM runtime_bridge.asi_runtime_bridge_tasks WHERE id = p_task_id AND client_id = p_client_id;
  END IF;
  RETURN jsonb_build_object(
    'deduplicated', v_deduplicated,
    'task', jsonb_build_object(
...
ALTER TABLE runtime_bridge.asi_runtime_bridge_tasks ENABLE ROW LEVEL SECURITY;
ALTER TABLE runtime_bridge.asi_runtime_bridge_owner_gates ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE runtime_bridge.asi_runtime_bridge_tasks FROM PUBLIC, anon, authenticated;
REVOKE ALL ON TABLE runtime_bridge.asi_runtime_bridge_owner_gates FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE ON TABLE runtime_bridge.asi_runtime_bridge_tasks TO service_role;
GRANT SELECT, INSERT, UPDATE ON TABLE runtime_bridge.asi_runtime_bridge_owner_gates TO service_role;

REVOKE ALL ON FUNCTION runtime_bridge.submit_asi_runtime_bridge_task(TEXT, TEXT, TEXT, TEXT, JSONB, TEXT) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION runtime_bridge.claim_asi_runtime_bridge_task(TEXT, TEXT, INTEGER) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION runtime_bridge.heartbeat_asi_runtime_bridge_task(TEXT, TEXT, UUID, UUID, INTEGER) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION runtime_bridge.complete_asi_runtime_bridge_task(TEXT, TEXT, UUID, UUID, JSONB) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION runtime_bridge.gate_asi_runtime_bridge_task(TEXT, TEXT, UUID, UUID, JSONB) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION runtime_bridge.expire_asi_runtime_bridge_owner_gates(TEXT) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION runtime_bridge.fail_asi_runtime_bridge_task(TEXT, TEXT, UUID, UUID, BOOLEAN, TEXT) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION runtime_bridge.decide_asi_runtime_bridge_owner_gate(TEXT, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION runtime_bridge.submit_asi_runtime_bridge_task(TEXT, TEXT, TEXT, TEXT, JSONB, TEXT) TO service_role;
GRANT EXECUTE ON FUNCTION runtime_bridge.claim_asi_runtime_bridge_task(TEXT, TEXT, INTEGER) TO service_role;
GRANT EXECUTE ON FUNCTION runtime_bridge.heartbeat_asi_runtime_bridge_task(TEXT, TEXT, UUID, UUID, INTEGER) TO service_role;
GRANT EXECUTE ON FUNCTION runtime_bridge.complete_asi_runtime_bridge_task(TEXT, TEXT, UUID, UUID, JSONB) TO service_role;
GRANT EXECUTE ON FUNCTION runtime_bridge.gate_asi_runtime_bridge_task(TEXT, TEXT, UUID, UUID, JSONB) TO service_role;
GRANT EXECUTE ON FUNCTION runtime_bridge.expire_asi_runtime_bridge_owner_gates(TEXT) TO service_role;
GRANT EXECUTE ON FUNCTION runtime_bridge.fail_asi_runtime_bridge_task(TEXT, TEXT, UUID, UUID, BOOLEAN, TEXT) TO service_role;
GRANT EXECUTE ON FUNCTION runtime_bridge.decide_asi_runtime_bridge_owner_gate(TEXT, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT) TO service_role;

CREATE POLICY asi_runtime_bridge_tasks_service_role
  ON runtime_bridge.asi_runtime_bridge_tasks FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY asi_runtime_bridge_gates_service_role
  ON runtime_bridge.asi_runtime_bridge_owner_gates FOR ALL TO service_role USING (true) WITH CHECK (true);
