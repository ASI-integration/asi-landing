...
-- Does not rewrite 20260909170000_runtime_bridge_schema_free_tier.sql or
-- 20260912210000_runtime_bridge_single_lane_admission_v1.sql.
-- Do not apply to production.
--
-- Closes the same distributed crash window documented in
...
--
-- Rollback:
--   REVOKE/DROP FUNCTION runtime_bridge.reconcile_asi_runtime_bridge_owner_gate(...).
--   No column/index changes to roll back.

CREATE FUNCTION runtime_bridge.reconcile_asi_runtime_bridge_owner_gate(
  p_client_id TEXT,
  p_runner_id TEXT,
  p_task_id UUID,
...
DECLARE
  ...
  v_existing runtime_bridge.asi_runtime_bridge_owner_gates%ROWTYPE;
  v_task_cycle TEXT;
...
BEGIN
  ...
  FROM runtime_bridge.asi_runtime_bridge_owner_gates
...
REVOKE ALL ON FUNCTION runtime_bridge.reconcile_asi_runtime_bridge_owner_gate(TEXT, TEXT, UUID, INTEGER, UUID, JSONB) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION runtime_bridge.reconcile_asi_runtime_bridge_owner_gate(TEXT, TEXT, UUID, INTEGER, UUID, JSONB) TO service_role;
